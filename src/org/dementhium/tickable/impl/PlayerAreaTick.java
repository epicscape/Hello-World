...
public class PlayerAreaTick extends Tick {
	...
	public void execute() {
		if (player.isOnline()) {
			...
			System.out.println("Ticky"+player.getUsername()+", "+player.getLocation().toString());
			if (player.getLocation().getX() >= 3349 && player.getLocation().getY() >= 3262 && player.getLocation().getX() <= 3387 && player.getLocation().getY() <= 3280){
				...
				if (player.getLocation().getX() >= 3333 && player.getLocation().getY() >= 3244 && player.getLocation().getX() <= 3358 && player.getLocation().getY() <= 3258){
					System.out.println("Ur in");
					updateWildernessState(true);
					ActionSender.sendPlayerOption(player, "Attack", 1, false);
				}
			}
			/*if (World.getWorld().getAreaManager().getAreaByName("Duel").contains(player.getLocation())) {
				System.out.println("induel");
				if (!inDuelArena) {
