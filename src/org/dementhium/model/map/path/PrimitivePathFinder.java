        ...
        for (int ii = 0; ii < max; ii++) {
            ...
            for (int i = 0; i < xLength; i++) {
                for (int i2 = 0; i2 < yLength; i2++) {
                    if (diffX < 0 && diffY < 0) {
                        if ((Region.getClippingMask(currentX + i - 1, currentY
                                + i2 - 1, height) & 0x128010e) != 0
                                || (Region.getClippingMask(currentX + i - 1,
                                currentY + i2, height) & 0x1280108) != 0
                                || (Region.getClippingMask(currentX + i,
                        ...
                        if ((Region.getClippingMask(currentX + i, currentY + i2
        ...
        for (int i = 0; i < stepCount; i++) {
            ...
            if (dstDir != null && canMove(curLoc, dstDir, false)) {
                ...
                if (!ignoreLastStep || curX != dstX || curY != dstY) {
                    state.getPoints().add(Position.create(step.getX(), step.getY(), step.getZ()));
                } else {
                    break;
                }
            } else if (nullOnFail) {
