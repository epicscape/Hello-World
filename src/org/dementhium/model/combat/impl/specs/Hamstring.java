...
/**
 * Executes the Morrigan's throwing axe special attack: Hamstring.
 * @author Emperor
 *
 */
public class Hamstring extends SpecialAttack {
	
	/**
	 * The graphics.
	 */
	private static final Graphic GRAPHICS = Graphic.create(1838);
	...
	public boolean commenceSpecialAttack(Interaction interaction) {
		...
		int maximum = RangeFormulae.getRangeDamage(interaction.getSource(), 1.2);
		data.setDamage(Damage.getDamage(interaction.getSource(), interaction.getVictim(), CombatType.RANGE,RangeFormulae.getDamage(interaction.getSource(), interaction.getVictim(), 1.1, 1.2, 1.0)));
		...
		interaction.getSource().animate(10504);
