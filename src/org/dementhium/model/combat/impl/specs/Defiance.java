...
/**
 * Executes the Zanik's crossbow special attack: Defiance
 * @author Emperor
 *
 */
public class Defiance extends SpecialAttack {
	
	/**
	 * The graphics.
	 */
	private static final Graphic GRAPHICS = Graphic.create(1714);
	
	/**
	 * The special attack projectile GFX id.
	 */
	private static final short PROJECTILE_ID = 2001;
	...
	public boolean commenceSpecialAttack(Interaction interaction) {
		...
		data.setDamage(Damage.getDamage(interaction.getSource(), interaction.getVictim(), CombatType.RANGE,RangeFormulae.getDamage(interaction.getSource(), interaction.getVictim(), 1.2, 1.0, 1.0) + interaction.getSource().getRandom().nextInt(151)));
		data.getDamage().setMaximum(maximum + 150);
		interaction.setTicks((int) Math.floor(interaction.getSource().getLocation().distance(interaction.getVictim().getLocation()) * 0.3));
		interaction.getSource().animate(11359);
