...
/**
 * Executes the Dorgeshuun c'bow's special attack: Snipe.
 * @author Emperor
 *
 */
public class Snipe extends SpecialAttack {
	
	/**
	 * The graphics.
	 */
	private static final Graphic GRAPHICS = Graphic.create(705, 96 << 16);
	
	/**
	 * The special attack projectile GFX id.
	 */
	private static final short PROJECTILE_ID = 698;
	...
	public boolean commenceSpecialAttack(Interaction interaction) {
		...
		if (data.getAmmo().getItemId() != 8882) {
			interaction.getSource().getPlayer().sendMessage("You can only use this attack with bone bolts.");
			interaction.getSource().getCombatExecutor().reset();
			return false;
		}
		...
		data.setDamage(Damage.getDamage(interaction.getSource(), interaction.getVictim(), CombatType.RANGE,RangeFormulae.getDamage(interaction.getSource(), interaction.getVictim(), 1.2, 1.0, 0.952)));
		...
		ProjectileManager.sendProjectile(Projectile.create(interaction.getSource(), interaction.getVictim(), PROJECTILE_ID, 38, 36, 41, speed, 5, 11));
		interaction.setTicks((int) Math.floor(interaction.getSource().getLocation().distance(interaction.getVictim().getLocation()) * 0.3));
		interaction.getSource().animate(4199);
