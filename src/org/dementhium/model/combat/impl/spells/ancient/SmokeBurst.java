...
/** 
 * Handles the Smoke burst ancient spell.
 * @author Emperor
 *
 */
public class SmokeBurst extends MagicSpell {
	...
	public boolean castSpell(Interaction interaction) {
		...
		if (interaction.getVictim().getWalkingQueue().isMoving()) {
			...
			ProjectileManager.sendProjectile(Projectile.create(interaction.getSource(), interaction.getVictim(), 388, 43, 31, 51, speed, 16, 64));
		}
		...
		for (ExtraTarget m : interaction.getTargets()) {
			...
			if (m.getDamage().getHit() > -1) {
				m.getVictim().getPoisonManager().poison(interaction.getSource(), 20);
			}
