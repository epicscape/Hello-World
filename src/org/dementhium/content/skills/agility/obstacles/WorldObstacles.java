			...
			tick = new Tick(player.getLocation() == to ? 0 : 1) {
				public void execute() {
					if (player.getLocation() == to) {
						...
						forceWalk(player, Animation.create(2589), player.getLocation().getX() + (index == 1 ? -1 : 1), player.getLocation().getY(), 1, new int[] {0, 60, -1}, null);
						World.getWorld().submit(new Tick(2) {
							...
							public void execute() {
								...
								case 1:
									...
									break;
								case 2:
									forceWalk(player, Animation.create(2591), player.getLocation().getX() + (index == 1 ? -1 : 1), player.getLocation().getY(), 1, new int[] {20, 30, -1}, null);
									break;
								case 3:
									stop();
